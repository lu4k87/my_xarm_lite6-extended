...
class VRTeleopNode(Node):
    def __init__(self):
        ...
        # Not-Aus-Zustand vom robot_motion_handler (latched). Solange er
        # verriegelt ist, startet dieser Node Servo nicht wieder und faehrt nicht.
        latched_qos = QoSProfile(depth=1, reliability=ReliabilityPolicy.RELIABLE,
                                 durability=DurabilityPolicy.TRANSIENT_LOCAL)
        self.estop_latched = False
        self.create_subscription(Bool, '/ui/emergency_stop_active', self.estop_cb, latched_qos)
    ...
    def estop_cb(self, msg):
        ...
        if self.estop_latched and self.grip_pressed:
            self.halt('emergency stop latched')

    def ensure_servo_started(self):
        # Bei jedem neuen Griff: Servo kann inzwischen gestoppt worden sein
        # (MoveIt-Bahn, Scan, Not-Aus + Quittieren). start_servo auf einen
        # laufenden Servo ist unschaedlich.
        if self.servo_start_cli.service_is_ready():
    ...
    def controller_cb(self, msg):
        ...
        if curr_index and not self.index_pressed and not self.estop_latched:
            self.toggle_gripper()
        ...
        if curr_grip and self.estop_latched:
        ...
        if abs(thumb_x) > 0.1 and not self.estop_latched:
