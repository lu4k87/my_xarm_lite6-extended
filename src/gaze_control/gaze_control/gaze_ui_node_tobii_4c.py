...
class EyeRosNode(Node):
    def __init__(self):
        super().__init__('gaze_ui_ros2_node_tobii')
        ...
        self.tobii_sub = self.create_subscription(Point, '/tobii/gaze', self.tobii_callback, 10)
...
class EyeControlUI(QWidget):
    ...
    def init_eye_tracker(self):
        # Tobii 4C Setup: Keine Kamera, kein RTSP.
        # Die Gaze-Koordinaten kommen direkt via ROS 2 Topic (/tobii/gaze) rein.
        print("[Gaze UI] Tobii 4C Modus aktiv. Warte auf /tobii/gaze Topic...")
        pass
...
def main(args=None):
    ...
    ros_node = EyeRosNode()
    ui = EyeControlUI(ros_node)
    ros_node.ui_instance = ui
    ui.showFullScreen()
    app.exec_()
    rclpy.shutdown()
