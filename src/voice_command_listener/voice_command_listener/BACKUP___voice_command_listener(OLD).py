...
# Bereinigt den Text von Sonderzeichen, normalisiert Unicode und wandelt
# Umlaute um, um das Regex-Matching robuster zu machen.
# -------------------------------------------------------------------------
...
# Setzt eine Liste von Woertern wieder zu einem lesbaren Satz zusammen
# und behandelt Satzzeichen korrekt.
# -------------------------------------------------------------------------
...
class VoiceCommandListener(Node):
    ...
    def __init__(self):
        ...
        # cooldown_sec: Globale Wartezeit zwischen zwei Befehlen
        # refractory_sec: Wartezeit bevor der *gleiche* Befehl erneut akzeptiert wird
        ...
        # ---- Publisher Setup ----
        # Publisher fuer Steuerbefehle an den MoveToCoordinator
        ...
        # Sendet formatierten Text an das Web-UI / Display
        ...
        self.word_buffer = deque(maxlen=50)                     # Puffer fuer Wort-Stream
        self.last_any_trigger_ts = 0.0                          # Zeitstempel letzter beliebiger Trigger
        self.refractory_until = {c: 0.0 for c in self.COLORS}   # Sperrzeiten pro Farbe
        self.last_match_count = {c: 0 for c in self.COLORS}     # Zaehler fuer Regex-Matches
        self.present_last = {c: False for c in self.COLORS}     # Status im letzten Zyklus
        ...
        # ---- Subscriptions ----
        # Hoert auf verschiedene Whisper-Topics, um maximale Kompatibilitaet zu sichern
        if HAS_WHISPER_IDL:
        ...
        # ---- Services ----
        # Service um den letzten erkannten Befehl abzufragen
    ...
    def on_transcript_msg(self, msg):
        # Verarbeitet komplexe AudioTranscript Nachrichten
        # Sucht nach Textfeldern oder Wortlisten in der Nachricht
        for field in ("text", "transcript", "full", "full_text"):
            if hasattr(msg, field):
                ...
                if isinstance(val, str) and val:
                    ...
                    return
        # Fallback: Rekonstruktion aus Wort-Liste
    ...
    def on_transcript_string(self, msg: StringMsg):
        # Verarbeitet einfache String-Nachrichten
        if msg.data:
    ...
    def handle_text(self, text_raw: str):
        # 1. Pruefung des Globalen Cooldowns
        now = time.time()
        if (now - self.last_any_trigger_ts) < self.cooldown_sec: return

        # 2. Normalisierung des Inputs
        norm = normalize(text_raw).replace("grun", "gruen") # Spezifischer Fix fuer Whisper-Fehler
        
        present_now = {c: False for c in self.COLORS}
        match_count = {c: 0 for c in self.COLORS}
        
        # 3. Regex-Matching gegen alle Pattern
            ...
            if present_now[color]:
                ...
                if now < self.refractory_until[color]:
                    ...
                    if match_count[color] > self.last_match_count[color]:
                        ...
                        break # Nur einen Befehl pro Trigger-Zyklus erlauben
                    else:
    ...
    def emit_command(self, color: str, original: str):
        ...
        # Mapping fuer schoene UI-Ausgabe (Deutsch + Icons)
        color_map_de = {"red": "Rot", "green": "Gruen", "blue": "Blau"}
        color_icon = {"red": "🔴", "green": "🟢", "blue": "🔵"}
        display_color = color_map_de.get(color, color.capitalize())
        icon = color_icon.get(color, "❔")

        # 1. Feedback-String fuer UI und Terminal erstellen
        feedback_lines = [
            
            "✅ Sprachbefehl erkannt!",
            "                        ",
            f'Kommando: "{cmd_text}"',
            f"-> Roboter bewegt sich zu Objekt: {icon} ({display_color})",
            
        ]
        feedback_string_for_ui = "\n".join(feedback_lines) # Mit Zeilenumbruechen

        # 2. Im Terminal ausgeben (lokales Debugging)
        print(CLEAR_SCREEN, end='')
        print(feedback_string_for_ui)
        self.get_logger().debug(f'(Originales Transkript: "{original}")')

        # 3. Kommando auf /voice_cmd publishen (fuer MoveToCoordinator)
        cmd_msg = StringMsg()
        cmd_msg.data = cmd_text
        self.cmd_pub.publish(cmd_msg)
        self._last_cmd_text = cmd_text

        # 4. <<< NEU: Feedback-String auf /ui/voice_feedback publishen >>>
        # Dient zur Anzeige im Web-Frontend
        try:
            feedback_msg = StringMsg()
            feedback_msg.data = feedback_string_for_ui
