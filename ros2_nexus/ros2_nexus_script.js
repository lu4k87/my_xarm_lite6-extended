    ...
    function getDevSetupActions(mode) {
      ...
      let zedBringupCmd = "ros2 launch my_3d_vision_bringup zed_cam_rviz_pointcloud_tf_yolo_planned_grasp.launch.py use_zed_hardware:=false yolo_model:=yolov8l.pt yolo_model:=yolov8s.pt yolo_model:=my_yolo_model.pt";
      ...
      if (mode === "real") {
        ...
        zedBringupCmd = "ros2 launch my_3d_vision_bringup zed_cam_rviz_pointcloud_tf_yolo_planned_grasp.launch.py use_zed_hardware:=true yolo_model:=yolov8l.pt yolo_model:=yolov8s.pt yolo_model:=my_yolo_model.pt";
      }
      ...
      const actions = [
        ...
        { cmd: "ros2 run my_3d_vision_bringup yolo_3d_bbox_ip_cam.py", title: "IP Cam YOLO 3D BBox (Homography .123)" },
    ...
    const CMD_DETAILS = {
      ...
      "ros2 run my_3d_vision_bringup yolo_3d_bbox_for_zed_m.py": `<div style="font-size: 11px; color: var(--mut); margin-bottom: 4px;"><b>Included Source Files:</b></div><ul style="padding-left: 16px; margin: 0; font-size: 11px; color: var(--mut); line-height: 1.4;"><li><span class="badge badge-node" style="margin-right: 6px;"><svg viewBox="0 0 100 100" style="width: 10px; height: 10px; margin-right: 4px; vertical-align: -0.15em;" fill="currentColor"><g stroke="currentColor" stroke-width="8"><line x1="61.3" y1="38.7" x2="80" y2="20"/><line x1="39.7" y1="37.7" x2="25" y2="20"/><line x1="34" y1="50" x2="15" y2="50"/><line x1="50" y1="66" x2="50" y2="85"/></g><circle cx="50" cy="50" r="12" fill="none" stroke="currentColor" stroke-width="8"/><circle cx="80" cy="20" r="11" fill="currentColor"/><circle cx="25" cy="20" r="11" fill="currentColor"/><circle cx="15" cy="50" r="11" fill="currentColor"/><circle cx="50" cy="85" r="11" fill="currentColor"/></svg> NODE</span><span style="color: var(--c-node); font-size: 1.7em; font-weight: normal;"> yolo_3d_bbox_for_zed_m</span> <span style="float: right; opacity: 0.7;">(my_3d_vision_bringup)</span></li></ul>`,
      "ros2 run my_3d_vision_bringup yolo_3d_bbox_ip_cam.py": `<div style="font-size: 11px; color: var(--mut); margin-bottom: 4px;"><b>Included Source Files:</b></div><ul style="padding-left: 16px; margin: 0; font-size: 11px; color: var(--mut); line-height: 1.4;"><li><span class="badge badge-node" style="margin-right: 6px;"><svg viewBox="0 0 100 100" style="width: 10px; height: 10px; margin-right: 4px; vertical-align: -0.15em;" fill="currentColor"><g stroke="currentColor" stroke-width="8"><line x1="61.3" y1="38.7" x2="80" y2="20"/><line x1="39.7" y1="37.7" x2="25" y2="20"/><line x1="34" y1="50" x2="15" y2="50"/><line x1="50" y1="66" x2="50" y2="85"/></g><circle cx="50" cy="50" r="12" fill="none" stroke="currentColor" stroke-width="8"/><circle cx="80" cy="20" r="11" fill="currentColor"/><circle cx="25" cy="20" r="11" fill="currentColor"/><circle cx="15" cy="50" r="11" fill="currentColor"/><circle cx="50" cy="85" r="11" fill="currentColor"/></svg> NODE</span><span style="color: var(--c-node); font-size: 1.7em; font-weight: normal;"> yolo_3d_bbox_ip_cam</span> <span style="float: right; opacity: 0.7;">(my_3d_vision_bringup)</span></li></ul>`,
      ...
      sections.forEach((sec, secIndex) => {
        ...
        <div class="section" data-sec-id="${secIndex}" style="position: relative; border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 10px; background: linear-gradient(145deg, rgba(15, 15, 20, 0.8) 0%, rgba(5, 5, 8, 0.95) 100%); box-shadow: 0 30px 60px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.05); padding: 16px 28px; ${sec.style || ''}">
          ...
          <div class="actions-grid" data-sec-index="${secIndex}" style="grid-template-columns: 1fr; gap: 10px; position: relative; z-index: 1;">`;
            ...
            (sec.actions || []).forEach((a, aIndex) => {
                ...
                sectionHtml += `
            <div class="card-wrapper" style="width: 100%; max-width: 420px; margin: 0 auto;">
              <div class="action-card" data-type="dev" onclick="openLaunchModal(this.closest('.card-wrapper'), ${actionFunc}, '🚀 ${a.label} gestartet...', '${a.type}')" style="cursor: pointer; border-radius: 12px; background: rgba(0,0,0,0.5); border: 1px solid ${badgeParams.border}; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); position: relative; --card-accent: linear-gradient(90deg, ${badgeParams.primary} 0%, transparent 50%); width: 100%; max-width: 420px; margin: 0 auto;">
                ...
                <div class="action-btn" style="pointer-events: none; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 9px 12px; position: relative; z-index: 1;">
                  <div style="display: flex; align-items: center; justify-content: center; gap: 12px; width: 100%; position: relative;">
                    ${cmdBadges.length ? `<div style="position: absolute; right: 14px; top: 50%; transform: translateY(-50%); display: flex; flex-direction: column; gap: 4px; align-items: flex-end;">${cmdBadges.map(b => `<span style="background: ${badgeParams.bg1}; border: 1px solid ${badgeParams.border}; border-radius: 6px; padding: ${cmdBadges.length > 1 ? '3px 7px' : '4px 9px'}; font-size: ${cmdBadges.length > 1 ? '10.5px' : '11.5px'}; font-weight: 800; letter-spacing: 1.5px; color: ${badgeParams.primary}; text-shadow: 0 0 12px ${badgeParams.glow}; white-space: nowrap;">${b}</span>`).join('')}</div>` : ''}
                    <div style="background: linear-gradient(135deg, ${badgeParams.bg1}, ${badgeParams.bg2}); border: 1px solid ${badgeParams.border}; border-radius: 8px; padding: 6px 11px; display: flex; flex-direction: column; align-items: center; justify-content: center; box-shadow: 0 8px 20px rgba(0,0,0,0.4), inset 0 0 12px ${badgeParams.bg1};">
                      <span style="color: ${badgeParams.primary}; font-size: 9px; font-weight: 800; letter-spacing: 2px;"><i class="${badgeIcon}" style="margin-right: 6px;"></i>${bText1}</span>
                      <span style="color: #fff; font-size: 11.5px; font-weight: 900; letter-spacing: 1.5px;">${bText2}</span>
                    </div>
                    <div style="display: flex; flex-direction: column; text-align: left;">
                      <span style="font-size: 19px; font-weight: 900; color: #fff; letter-spacing: -0.5px; line-height: 1.1; text-shadow: 0 0 40px ${badgeParams.glow};">${cmdTitle}</span>
                      <span style="font-size: 10px; color: ${badgeParams.primary}; font-weight: 800; letter-spacing: 3px; text-transform: uppercase; margin-top: 2px; opacity: 0.9;">${cmdSub}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="section" data-sec-id="${secIndex}" style="${extraStyle}position: relative; border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 10px; background: linear-gradient(145deg, rgba(15, 15, 20, 0.8) 0%, rgba(5, 5, 8, 0.95) 100%); box-shadow: 0 30px 60px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.05); padding: 16px 28px;">
          ...
          <div class="actions-grid" style="grid-template-columns: 1fr; gap: 10px; position: relative; z-index: 1;" data-sec-index="${secIndex}">
            <div class="card-wrapper" style="width: 100%; max-width: 420px; margin: 0 auto;">
              <div class="action-card" data-type="sys" onclick="openLaunchModal(this.closest('.card-wrapper'), TABS[window.currentTab][${secIndex}].actions, '🚀 ${sec.title.replace(/'/g, "\\'")} gestartet...', 'sec_${secIndex}')" style="cursor: pointer; border-radius: 12px; background: rgba(0,0,0,0.5); border: none; box-shadow: inset 0 0 0 1px ${secColor}40; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); position: relative; --card-accent: linear-gradient(90deg, ${secColor} 0%, transparent 50%); width: 100%; max-width: 420px; margin: 0 auto;">
                ...
                <div class="action-btn" style="pointer-events: none; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 9px 12px; position: relative; z-index: 1;">
                   <div style="display: flex; align-items: center; justify-content: center; gap: 12px; width: 100%; position: relative;">
                    ${sec.badge ? `<span style="position: absolute; right: 14px; top: 50%; transform: translateY(-50%); background: ${secColor}26; border: 1px solid ${secColor}66; border-radius: 6px; padding: 4px 9px; font-size: 11.5px; font-weight: 800; letter-spacing: 1.5px; color: ${secColor}; text-shadow: 0 0 12px ${secColor}66; white-space: nowrap;">${sec.badge}</span>` : ''}
                    <div style="background: linear-gradient(135deg, ${secColor}26, ${secColor}0D); border: 1px solid ${secColor}66; border-radius: 8px; padding: 6px 11px; display: flex; flex-direction: column; align-items: center; justify-content: center; box-shadow: 0 8px 20px rgba(0,0,0,0.4), inset 0 0 12px ${secColor}26;">
                      <span style="color: ${secColor}; font-size: 9px; font-weight: 800; letter-spacing: 2px;"><i class="fa-solid fa-layer-group" style="margin-right: 6px;"></i>MODULE</span>
                      <span style="color: #fff; font-size: 11.5px; font-weight: 900; letter-spacing: 1.5px;">SEQUENCE</span>
                    </div>
                    <div style="display: flex; flex-direction: column; text-align: left;">
                      ${sec.title.toUpperCase() === 'CONTROLLERS (INPUT -> MOVEIT SERVO)' ? 
                        `<span style="font-size: 15.5px; font-weight: 900; color: #fff; letter-spacing: -0.3px; line-height: 1.15; text-shadow: 0 0 40px ${secColor}33;">Choose: Robot Control MoveIt - Input Device</span>` : 
                        `<span style="font-size: 17px; font-weight: 900; color: #fff; letter-spacing: -0.5px; line-height: 1.1; text-shadow: 0 0 40px ${secColor}33;">${sec.title}</span>`}
                      <span style="font-size: 10px; color: ${secColor}; font-weight: 800; letter-spacing: 3px; text-transform: uppercase; margin-top: 2px; opacity: 0.9;">Contains ${actionCount} Components</span>
                    </div>
                  </div>
                </div>
              </div>
