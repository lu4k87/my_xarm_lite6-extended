    ...
    async function runClientSetup() {
      const actions = [
        ...
        { cmd: "ros2 launch whisper_bringup bringup.launch.py silero_vad_use_cuda:=False", title: "Whisper Bringup (Voice AI) - CPU Mode" },
        { cmd: "ros2 run voice_command_listener listener", title: "Voice Command Listener" },
        { cmd: "python3 -m http.server 8081 -d src/robot_control_web_ui & sleep 1 && (google-chrome --user-data-dir=$HOME/.robot_control_profile --class=\"robot-control-ui\" --start-maximized --app=http://127.0.0.2:8081/index.html || chromium-browser --user-data-dir=$HOME/.robot_control_profile --class=\"robot-control-ui\" --start-maximized --app=http://127.0.0.2:8081/index.html || xdg-open http://127.0.0.2:8081/index.html) & wait", title: "Robot Control Web UI SERVER PORT: 8081" }
      ];
    ...
    const CMD_DETAILS = {
      ...
      "ros2 launch whisper_bringup bringup.launch.py silero_vad_use_cuda:=False": `<div style="font-size: 11px; color: var(--mut); margin-bottom: 4px;"><b>Included Source Files:</b></div><ul style="padding-left: 16px; margin: 0; font-size: 11px; color: var(--mut); line-height: 1.4;"><li><span style="color: var(--c-launch);">bringup.launch.py</span> (Main)<ul style="padding-left: 16px; margin: 2px 0 0 0;"><li><span style="color: var(--accent);">audio_listener.py</span> (Mic Stream)</li><li><span style="color: var(--accent);">inference.cpp</span> (Whisper CPU Engine)</li></ul></li></ul>`,
