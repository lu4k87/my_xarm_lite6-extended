...
* **`my_zed_tf_bringup`**
    ...
    * **Funktionsweise:** Führt `zed_camera.launch.py` aus, um den nativen Stereolabs-Treiber zu initialisieren (jetzt in `common.yaml` auf den hochauflösenden `NEURAL` Tiefenmodus konfiguriert, der die RTX GPU für dichte KI-Punktwolken nutzt) und sendet simultan eine statische Transformation von `link_base` zu `zed_camera_link`. Standardmäßig ist dies für ein physisches Stativ-Setup parametrisiert (x=0.75m, y=0.0m, z=0.4m, pitch=45°, yaw=180°), wodurch die Kamera in RViz2 direkt in das Weltkoordinatensystem des Roboters eingebunden wird. Zusätzlich laufen zwei eigene Python-Skripte: `zed_stand_publisher.py` generiert dynamisch das 3D-Stativ und das ZED-Kameramesh (`ZEDM.stl`). Parallel verarbeitet die eigenständige Node `zed_yolo_3d_bbox.py` den RGB- und Depth-Stream der Kamera mit GPU-Beschleunigung für YOLOv8. Sie extrahiert die echte 3D-Punktwolke jedes erkannten Objekts, transformiert diese in das Weltkoordinatensystem des Roboters, filtert Tiefenrauschen ("Flying Pixels") robust über das 2. und 98. Perzentil und isoliert das Objekt strikt vom Hintergrund. Das Ergebnis sind zitterfreie, millimetergenaue 3D-Bounding-Boxen, die live in RViz publiziert werden – inklusive eines kombinierten Text-Markers, der den Objektnamen und die X/Y/Z-Koordinaten sauber über dem Objekt einblendet. Geister-Marker werden durch einen sauberen DELETEALL-Mechanismus komplett verhindert.
