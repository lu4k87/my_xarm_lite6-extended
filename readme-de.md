...
* **`my_zed_tf_bringup`**
    ...
    * **Funktionsweise:** Führt `zed_camera.launch.py` aus, um den nativen Stereolabs-Treiber zu initialisieren (jetzt in `common.yaml` auf den hochauflösenden `NEURAL` Tiefenmodus konfiguriert, der die RTX GPU für dichte KI-Punktwolken nutzt) und sendet simultan eine statische Transformation von `link_base` zu `zed_camera_link`. Standardmäßig ist dies für ein physisches Stativ-Setup parametrisiert (x=0.75m, y=0.0m, z=0.36m, pitch=45°, yaw=180°), wodurch die Kamera in RViz2 direkt in das Weltkoordinatensystem des Roboters eingebunden wird. Zusätzlich laufen zwei eigene Python-Skripte: `zed_stand_publisher.py` generiert mathematisch exakt das 3D-Modell eines 20x20mm V-Slot Aluminiumprofils sowie das ZED-Kameramesh (`ZEDM.stl`). Parallel verarbeitet die eigenständige Node `zed_yolo_3d_bbox.py` den vollen, unbeschnittenen RGB- und Depth-Stream der Kamera mit GPU-Beschleunigung und dem hochgenauen **YOLOv8 Large (`yolov8l.pt`)** Modell. Sie extrahiert die echte 3D-Punktwolke jedes erkannten Objekts, transformiert diese in das Weltkoordinatensystem des Roboters, filtert den physischen Roboter-Fuß (`X < 0.25`) und Tiefenrauschen ("Flying Pixels") robust über das 2. und 98. Perzentil heraus und isoliert das Objekt strikt vom Hintergrund. Das Ergebnis sind zitterfreie, millimetergenaue 3D-Bounding-Boxen, die **hart auf die Tischebene (`Z=0.0`) geerdet** sind und live als **extrem feine 1mm Wireframe-Kanten (`LINE_LIST`)** in RViz publiziert werden. Direkt über der Box werden mehrere, präzise übereinander gestapelte Text-Label eingeblendet. Diese extrem feinen Labels zeigen den Objektnamen (Weiß) sowie die X/Y/Z-Koordinaten in den exakten farblichen Kodierungen der RViz-Achsen (Rot, Grün, Blau), formatiert mit Unterstrichen (z.B. `X:_407_mm`), um bekannte Text-Justierungs-Bugs in RViz zu umgehen. Geister-Marker werden durch einen sauberen DELETEALL-Mechanismus komplett verhindert.
