...
5. [⚙️ Core Features & ROS 2 Nodes](#chapter-5)
   ...
   - [🌌 5.5 Funktion: Digital Twin & Simulation (NVIDIA Isaac Sim)](#subchapter-5-5)
...
- 🛠️ **Services:** `/compute_ik` (IK Verifizierung), `/move_action` (MoveIt OMPL Planer), `/ui/execute_move_to_pose` (Servo Fallback).
...
- ✨ **Core Features:** 
  ...
  - **Interaktives UI & Layout-Optimierung:** Die Benutzeroberfläche ist logisch strukturiert (Cartesian Jogging oben, Telemetrie darunter) und passt sich lückenlos an den Inhalt an. Sie beinhaltet dynamische, interaktive Elemente wie den Whisper AI "Start Listening"-Button. Dieser operiert nun im intuitiven **Push-to-Talk (PTT)** Modus: Solange der Button gedrückt gehalten wird, läuft die Echtzeit-Sprachaufnahme (bis zu 15s). Beim Loslassen wird die Aufnahme sofort gestoppt und ausgewertet. Um höchste Stabilität zu garantieren, löscht ein strikter C++-Zeitstempel-Filter im Backend alte Überreste im Audio-Puffer vollautomatisch heraus. Dies verhindert mathematisch sicher "Geister"-Kommandos und Doppel-Auslösungen bei schnellen Aufnahmen hintereinander. Das finale erkannte Transkript wird zur direkten Ausführung an das ROS-Backend übermittelt, was externe Whisper-Testskripte vollständig überflüssig macht.
...
### 🌌 <a id="subchapter-5-5"></a> 5.5 Funktion: Digital Twin & Simulation (NVIDIA Isaac Sim)
