...
* **`my_zed_tf_bringup`**
    ...
    * **Funktionsweise:** Führt `zed_camera.launch.py` aus, um den nativen Stereolabs-Treiber zu initialisieren und sendet simultan eine statische Transformation von `link_base` zu `zed_camera_link`. Standardmäßig ist dies für ein physisches Stativ-Setup parametrisiert (x=0.75m, y=0.0m, z=0.4m, pitch=45°, yaw=180°), wodurch die Kamera in RViz2 direkt in das Weltkoordinatensystem des Roboters eingebunden wird. Zusätzlich laufen zwei eigene Python-Skripte: `zed_stand_publisher.py` generiert dynamisch das 3D-Stativ und das ZED-Kameramesh (`ZEDM.stl`). Parallel verarbeitet die eigenständige Node `zed_yolo_3d_bbox.py` den RGB- und Depth-Stream der Kamera. Sie nutzt ein offizielles YOLOv8-Modell für die 2D-Objekterkennung (die `yolov8s.pt` Modellgewichte werden beim ersten Start automatisch heruntergeladen) und projiziert die gefundenen Pixelkoordinaten mittels der gemessenen Tiefenwerte und Kameramatrix in millimetergenaue 3D-Bounding-Boxen, welche live in RViz publiziert werden.
