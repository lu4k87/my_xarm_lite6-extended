...
* **`my_zed_tf_bringup`**
    * **Zweck:** Einheitliche Initialisierung der ZED Mini Kamera, TF-Broadcasting, Pointcloud-Optimierung und 3D-Visualisierung.
    * **Aufgabe:** Startet den `zed_wrapper` Node sicher, kombiniert ihn mit dem statischen TF-Publisher, filtert die Punktwolken und generiert die 3D-Kameramodelle in RViz.
    * **Funktionsweise:** Führt `zed_camera.launch.py` aus, um den nativen Stereolabs-Treiber zu initialisieren und sendet simultan eine statische Transformation von `link_base` zu `zed_camera_link`. Standardmäßig ist dies für ein physisches Stativ-Setup parametrisiert (x=0.75m, y=0.0m, z=0.5m, pitch=45°, yaw=180°), wodurch die Kamera in RViz2 direkt in das Weltkoordinatensystem des Roboters eingebunden wird. Zusätzlich laufen zwei eigene Python-Skripte: `zed_stand_publisher.py` generiert dynamisch das 3D-Stativ und das ZED-Kameramesh (`ZEDM.stl`). Parallel verarbeitet `pointcloud_optimizer.py` die Rohdaten der Kamera. Dieser Node wurde vollständig auf reines Numpy 2.x umgeschrieben (ohne Open3D/SciPy-Abhängigkeiten) und nutzt einen extrem schnellen Hash-basierten Voxel-Downsampling-Algorithmus, um die Performance zu maximieren und die Pointcloud korrekt vom Optical-Frame in das ROS-Frame zu rotieren.
