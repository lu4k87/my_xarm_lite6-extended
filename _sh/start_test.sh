...
gnome-terminal --title="[PROGRAM]: Webserver (Dashboard UI)" -- bash -c "source /opt/ros/humble/setup.bash; source ~/dev_ws/install/setup.bash; cd ~/dev_ws/src/dashboard_monitoring; python3 -m http.server 8080; exec bash"

# 3. Terminal: Workspace Analyzer
gnome-terminal --title="[SCRIPT]: Workspace Analyzer" -- bash -c "source /opt/ros/humble/setup.bash; source ~/dev_ws/install/setup.bash; cd ~/dev_ws/src/dashboard_monitoring; python3 workspace_analyzer.py; exec bash"
