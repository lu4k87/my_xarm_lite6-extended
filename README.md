...
* **`scan_trajectory_node`** *(in package `xarm_moveit_servo` | REAL & FAKE modes)*
    ...
    * **Task:** Generates a continuous half-circle path (radius 150mm) over a target object (`X=300, Y=0, Z=0`) and computes the exact camera orientation via look-at logic (quaternions).
    * **Operation:** Offers the `/ui/execute_scan_trajectory` service (e.g., triggered by the "Vision Scan" button in the RViz Control Panel). The node first pauses MoveIt Servo (`/servo_server/stop_servo`), calculates 40 Cartesian waypoints along a half-circle arc (±80° around the focus point, Z increases from 100mm to 300mm), and uses the MoveIt 2 C++ API (`MoveGroupInterface`) for smooth path planning (`computeCartesianPath`). The camera circles the object while the TCP Z-axis remains perfectly focused on it. Upon completion, MoveIt Servo is seamlessly reactivated. Since the node utilizes the `move_group` action server integrated into the launch system, it is 100% compatible in both FAKE and REAL modes.

* **`move_group` (MoveIt 2 Action Server)** *(integrated in all Servo launch files | REAL & FAKE modes)*
    * **Purpose:** Provision of the central MoveIt 2 path planning server.
    * **Task:** Enables the use of the **RViz MotionPlanning plugin** (interactive waypoints, path planning, "Plan & Execute") **in parallel** with MoveIt Servo (Gamepad/Control Panel).
    * **Operation:** The `move_group` node is automatically started in the background when the system launches (FAKE or REAL). It provides action servers through which both the RViz MotionPlanning plugin and custom nodes (like the `scan_trajectory_node`) can plan and execute paths. **Dual-Mode Operation:** MoveIt Servo (real-time streaming via gamepad) and MoveIt Planning (path planning via RViz) utilize the same trajectory controller and automatically alternate. The MotionPlanning panel is already activated in the `servo.rviz` configuration – the interactive goal marker (orange robot) can be directly dragged and dropped to set target poses and plan paths.
