...
>   - **MoveIt Popup (Progress, Gizmo Target, Confirmation):** The popup sits at the top center of the viewport between the navigation gizmo and the SCENE panel (in viewports narrower than 780 px in its own full-width row). It shows the gizmo target (`TARGET X/Y/Z`, distance Δ to the real TCP) with the **Auto-Move** checkbox, and during a MoveTo the steps IK → PLAN → EXECUTE with live timers, a progress bar, rejected candidate paths and the result or error. With Auto-Move off, the popup appears after releasing the gizmo as "GIZMO TARGET" and only moves on *Execute path*; with the path preview active, the same button confirms the planned path. It hides itself after the motion (5 s on success, 12 s on failure). Fed by `/ui/moveit_motion_state`; the steps also appear as `[MoveIt]` lines in the log.
