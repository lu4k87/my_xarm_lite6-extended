...
>   - **Object Context Menu:** Clicking the red grasp sphere in the viewport or an entry of the object list opens a menu: *Approach from above* (MoveTo 80 mm above the grasp point, gripper facing down), *Grasp* (grasp routine) and *Disable / Enable collision for this object* (`/ui/set_object_collision`).
...
>   - **Collapsible Sections:** Every section of the Robot Control UI (camera streams, YOLO, 3D viewport, jogging, telemetry, speech control, TF Control Tuner, log) has a small chevron button at the top right of its header. Collapsed, only the header row stays visible; the state is remembered per section in the browser (`localStorage`). The button sits in the drag handle but never starts a drag.
>   - **MoveIt Popup (Progress, Gizmo Target, Confirmation):** The popup sits at the bottom center directly above the POSE panel. It shows the gizmo target (`TARGET X/Y/Z`, distance Δ to the real TCP) with the **Auto-Move** checkbox, and during a MoveTo the steps IK → PLAN → EXECUTE with live timers, a progress bar, rejected candidate paths and the result or error. With Auto-Move off, the popup appears after releasing the gizmo as "GIZMO TARGET" and only moves on *Execute path*; with the path preview active, the same button confirms the planned path. It hides itself after the motion (5 s on success, 12 s on failure). Fed by `/ui/moveit_motion_state`; the steps also appear as `[MoveIt]` lines in the log.
...
>   - **Color-Coded Console Log:** Live, scrollable log with syntax highlighting (axes, numbers, topics, units) and colored source tags (`[MoveIt]`, `[UI]`, `[SAFETY]` …), timestamp on hovering the tag. It only auto-scrolls while you are at the bottom and keeps the last 500 entries.
>
>
> ![Subscribes](https://img.shields.io/badge/Subscribes-orange?style=flat-square)
>
