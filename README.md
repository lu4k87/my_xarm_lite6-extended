...
>
> 🥽 **VR viewport (Robot Control UI in the headset):** The `8443` server also serves the complete **Robot Control UI** over HTTPS (`https://<PC-IP>:8443/`), where it connects to the WSS rosbridge on `9091`. The viewport header then shows 🥽 **Enter VR** and 👓 **Passthrough (AR, prepared)**. The headset renders the same Digital Twin (`js/twin/xr.js`): live robot, objects, collision objects, ghost and MoveIt plan. In the Quest browser the page starts scaled down – at 100 % browser zoom it looks like it otherwise does at 50 % (CSS zoom, detected via the user agent; `?uizoom=1` turns it off, `?uizoom=auto` back on).
