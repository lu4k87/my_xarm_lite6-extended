...
>> | `pub_resolution` | `NATIVE` | *Native 1080p full sensor resolution (~2,073,600 points/frame, zero downsampling).* |
...
> ```
>
> **Purpose & Task:** Seamlessly converts the detected 3D boxes into dynamic MoveIt `CollisionObject` messages. Instead of a solid block, it generates an **open-top cup shape** (5 ultra-thin 1mm walls). This allows the gripper to safely penetrate the bounding box from above for top-down grasps, while securely blocking lateral collisions. The side walls end `top_clearance` (parameter, default 0.03 m) below the object top: MoveIt Servo halts 2 cm before any collision geometry, so wall edges exactly at the top acted like a lid when approaching from above. `/ui/set_object_collision` permanently disables or re-enables the collision of a single object (context menu of the Robot Control UI).
>
>
> ![Subscribes](https://img.shields.io/badge/Subscribes-orange?style=flat-square)
>
...
> ```
> *(Launched via ROS 2 Nexus App: Vision Section)*
>
...
>> | *-* | *-* | *🛡️ **3-Layer Deduplication:** **(1)** Feedback text dedup, **(2)** Residual audio detection, **(3)** Global cooldown (3s).* |
...
> - **No Fixed Keep-Out Zone Around the Axis:** MoveTo used to reject every target with r < 125 mm below z = 280 mm. That zone was much larger than the truly unreachable area (e.g. r = 100 mm, z = 200 mm is reachable without problems). Now only the IK with collision checking (self-collision) and the planning decide; a truly impossible target fails with "IK calculation failed … out of reach or in collision".
> - **Path Preview (optional):** When enabled via `/ui/set_moveto_preview` (parameter `moveto_preview`, default off), MoveTo only plans (`plan_only`), publishes the path latched on `/ui/moveto_preview_path` (the Robot Control UI shows it as a ghost robot) and waits for `/ui/confirm_moveto_preview`. Confirmed, the arm executes exactly this path via `/execute_trajectory`; discarded, or without an answer after `moveto_preview_timeout` (60 s), it does not move. MoveIt Servo stays paused while waiting, and the emergency stop aborts here too.
...
> - **`http_robot_control_ui_p8081/js/`** (mainly `motion.js`, `safety.js`): The browser frontend (roslibjs, ES modules) of the Web Panel commands Initial Pose, Scans, absolute XYZ movements, and Emergency Stops through this node.
...
> ```
> *(Automatically started via Nexus Web Bringup)*
>
> **Purpose & Task:** Captures the native running X11 RViz window in real-time via `xwininfo` and `mss`, converts screen buffers into standard BGR8 ROS Image messages, and publishes them at 15 FPS to `/rviz_video/image_raw`. This allows the full 3D RViz scene to be streamed seamlessly into the Web UI via `web_video_server` (Port 8082) without requiring heavy client-side 3D WebGL rendering.
>
>
> ![Publishes](https://img.shields.io/badge/Publishes-green?style=flat-square)
>
>> | Topic / Interface | Msg Type | Description |
>> |---|---|---|
>> | **`/rviz_video/image_raw`** | `sensor_msgs/Image` | *Publishes the live screen capture stream of the RViz2 window.* |
...
>   - **E-Stop in the Header + Space Bar:** The emergency stop sits fixed in the status bar - independent of the drag-and-drop layout and reachable above the offline overlay. The **space bar** triggers it everywhere (except in text fields). While latched, the button pulses and a *Reset* button appears next to it. While the emergency stop is latched, the motion buttons (Initial Pose, Scan Position, Object Scan, Go) are greyed out and disabled - clicking them plays neither the click sound nor a voice - and `motionAllowed()` blocks every motion, including voice commands and the gizmo.
...
>   - **Execute Button: Ghost, then Play:** With the preview on, the Execute button in the MoveIt popup shows the ghost icon while a click still creates the preview; as soon as the ghost path is shown (phase `confirm`), it switches back to Play - the second click moves the robot.
>   - **MOTION Buttons with Confirmation:** With *Auto-Move* off, Initial Pose, Scan Position and Object Scan in the MOTION tab behave like the viewport gizmo: the click opens the confirm popup ("INITIAL POSE" / "SCAN POSITION" / "OBJECT SCAN"), the robot only moves after *Execute* (X discards). With the preview on, all three first show a ghost path (Initial Pose then runs through MoveIt instead of the direct joint trajectory). With *Auto-Move* on they move immediately; the emergency stop and voice commands always act immediately.
...
>   - **MoveIt Popup (Progress, Gizmo Target, Confirmation):** The popup sits at the bottom center of the viewport above the POSE HUD tab with generous spacing and a sleek, translucent glassmorphism background. It integrates live TCP Gizmo target coordinates (`TARGET X/Y/Z`, distance Δ to real TCP) with the **Auto-Move** checkbox, and during a MoveTo displays the steps IK → PLAN → EXECUTE with live timers, progress bar, rejected candidate paths and the result or error. With Auto-Move off, the popup appears after releasing the gizmo as "GIZMO TARGET" and moves on *Execute path*; with path preview active, the same button confirms the planned path. It auto-hides after motion (5 s on success, 12 s on failure). Fed by `/ui/moveit_motion_state`; steps also appear as `[MoveIt]` lines in the log.
...
>> | **`/ui/emergency_stop_active`** | `std_msgs/Bool` | *Latched emergency stop (reset button in the header).* |
>
>
> ![Publishes](https://img.shields.io/badge/Publishes-green?style=flat-square)
>
...
**Ubuntu App Integration (One-Click Installer):** Both the **ROS 2 Nexus App** and the **Robot Control UI** can be registered as native Ubuntu applications with custom high-resolution icons and dedicated Chrome `--app` profiles. Simply run the automated installer script:
```bash
cd ~/dev_ws/ros2_nexus && bash install_app.sh
```
This automatically configures the paths, copies desktop shortcuts to `~/.local/share/applications/`, and updates the desktop database. Afterwards, you can launch **"ROS 2 Nexus"** and **"Robot Control UI"** directly from the Ubuntu Activities application menu or pin them to the Ubuntu dock.
...
1. Navigate through the available tabs: `Nodes / Launch` (all node and launch buttons), `Pub MSG on Topic` (publish messages to a topic by hand), `ROS Info` (live environment and network information) and `System` (terminal and system commands).
2. Inside the `Nodes / Launch` tab the buttons are grouped into collapsible sections — `AUTOMATED SYSTEM BRINGUP`, `Lite6 Fake / Lite6 - Moveit Servo (+Rviz2)`, `Controllers (Input -> Moveit Servo)`, `Visualization (Rviz2)`, `Vision (Cameras + CV)`, `Workspace Analyzer Backend`, `Frontend & Browser`, `Client / Server Control Bringup`, `NVIDIA Isaac Sim` and `MoveIt Planning (OMPL Server)`. The ZED camera driver, for instance, lives in **`Vision (Cameras + CV)`**.
...
| :--- | :--- | :--- | :--- |
| **`5000`** | HTTP / SocketIO | **ROS 2 Nexus Web Backend** | *Central process manager & web console.* |
...
| **`8554`** | RTSP | **Tobii Glasses 3 Stream** | *Video & JSON Gaze data transmission.* |
...
```mermaid
flowchart TD
    WEB["Web Browser Frontend<br/>(Port 5000)"] --> FLASK["Flask & SocketIO Server"]
...
Once the nodes are launched via ROS 2 Nexus, the live state of the system can be monitored using the **Dashboard Monitoring UI**. This is a web-based real-time UI, which fuses static source code analysis with live ROS 2 network telemetry into a unified monitoring interface.
...
│   ├── ros2_nexus_web.png                                                 # ROS 2 Nexus Web Launcher UI preview
...
├── isaacsim/                                                              # NVIDIA Isaac Sim simulation assets & configurations
...
│   │   └── dashboard_index.html                                           # Real-time reactive web dashboard UI (Port 8080)
...
│   │   │   └── twin/digital_twin.js                                       # Three.js 3D WebGL Digital Twin & scene objects
...
│   │   ├── CMakeLists.txt
│   │   ├── package.xml
│   │   └── launch/web_video_server.launch.py                              # Launches web_video_server & rviz_window_streamer
...
│   │   ├── config/
│   │   │   ├── grasping_params.yaml                                       # Grasp planning offsets, speeds & timeouts
│   │   │   ├── perception_params.yaml                                     # YOLO thresholds, EMA smoothing & filter params
...
│   ├── rviz_window_streamer/                                               # 📹 Python/FFmpeg: X11 RViz window capture to MJPEG stream
...
│   │   └── vr_quest3_teleop_node.py                                       # VR 6-DoF controller pose to MoveIt Servo bridge
...
- **Why it was replaced:** This was error-prone, hard to debug, and unintuitive for new users. It was entirely replaced by **ROS 2 Nexus**, a web-based orchestrator that securely manages process lifecycles, aggregates logs, and allows one-click bringup from any device.
